...
open class BotBase(val queueCommand: String, val quickRefresh: Int = 10000) {
    ...
    fun onClientTick(ev: ClientTickEvent) {
        ...
        if (toggled) {
            if (mc.thePlayer != null && mc.thePlayer.hurtTime == 10) {
                val bot = DuckDueller.bot
                if (bot is Sumo) {
                    bot.onClientDamage()
                }
            }
        ...
        if (KeyBindings.toggleBotKeyBinding.isPressed) {
            toggle()
            ChatUtils.info("Charizard has been toggled ${if (toggled()) "${EnumChatFormatting.GREEN}on" else "${EnumChatFormatting.RED}off"}")
            if (toggled()) {
